...
event_counts as (
    select
        ...
        case
            when page_view_counts >= 2 or page_ping_counts >= 2 then true else false
        end as engaged
    from (
        select
            domain_sessionid,
    ...
    left join (
        select
            domain_sessionid,
            countif(event_name = 'page_view') page_view_counts,
            countif(event_name = 'page_ping') page_ping_counts
        from {{ ref('snowplow_web_base_events_this_run') }}
        where domain_sessionid is not null
        and event_name is not null
        group by domain_sessionid  
    ) using (domain_sessionid)
    group by
        domain_sessionid, engaged
)
...
left join
    {{ ref('geo_country_mapping') }} e on a.geo_country = e.alpha_2
where
